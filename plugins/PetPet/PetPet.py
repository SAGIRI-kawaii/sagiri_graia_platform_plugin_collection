...
async def petpet_generator(app: GraiaMiraiApplication, message: MessageChain, member: Member, group: Group):
    ...
    if message.has(At) and message_text.startswith("摸") or message_text.startswith("摸 "):
        ...
        try:
            await app.sendGroupMessage(
                group,
                MessageChain.create([
                    Image.fromLocalFile(f"./plugins/PetPet/tempPetPet-{message.get(At)[0].target}.gif")
                ])
            )
        except AccountMuted:
            pass
...
frames = tuple([f'./plugins/PetPet/PetPetFrames/frame{i}.png' for i in range(5)])
...
async def petpet(member_id, flip=False, squish=0, fps=20) -> None:
    ...
    # 输出
    await save_gif(gif_frames, f'./plugins/PetPet/tempPetPet-{member_id}.gif', fps=fps)
